...
RUN mkdir -p /opt/couchdb && \
    ...
    rpm -e --allmatches $(rpm -qa --qf "%{NAME}\n" | grep -v -E "bash|coreutils|filesystem|glibc$|libacl1|libattr1|libcap2|libgcc_s1|libgmp|libncurses|libpcre|libreadline|libselinux|libstdc\+\+|openSUSE-release|system-user-root|terminfo-base|sed|libz1|libjitterentropy3|libopenssl3|crypto-policies|libicu|timezone|curl|libnghttp|libidn|libzstd|libpsl|libunistring") && \
    rm -Rf /etc/zypp && \
    rm -Rf /usr/lib/zypp* && \
