...
ARG MEND_URL
ARG MEND_USER_KEY
RUN curl -L https://downloads.mend.io/cli/linux_amd64/mend -o /tmp/mend && chmod +x /tmp/mend && \
    /tmp/mend update && \
    /tmp/mend dep -d "/usr/src/apache-couchdb-$COUCHDB_VERSION" -e -s "OpenScape UC Portfolio//OSEM third party applications//couchdb-osem-sourcecode - $COUCHDB_VERSION-r$RELEASE_VERSION" -u --non-interactive
...
RUN mkdir -p /opt/couchdb && \
    ...
    rpm -e --allmatches $(rpm -qa --qf "%{NAME}\n" | grep -v -E "bash|coreutils|filesystem|glibc$|libacl1|libattr1|libcap2|libgcc_s1|libgmp|libncurses|libpcre|libreadline|libselinux|libstdc\+\+|openSUSE-release|system-user-root|terminfo-base|sed|libz1|libjitterentropy3|libopenssl3|crypto-policies|libicu|timezone|ca-certificates") && \
    rm -Rf /etc/zypp && \
    rm -Rf /usr/lib/zypp* && \
